...
class SimpleIntegrationSwitzerlandDiplomaticTest {
    @Autowired
    ...
    private final String PATH = "/api/v1/switzerland/diplomatic/plates";
    ...
    void getRegionHandle_whenGetSwitzerland_thenStatus200() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders
                ...
                .andExpect(jsonPath("$.region", equalTo("4")))
                .andExpect(jsonPath("$.description").isNotEmpty())
                .andExpect(jsonPath("$.description", equalTo("Australia")));
    }

    @Test
    void getRegionHandle_whenGetSwitzerlandByDescription_thenStatus200() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders
                        .get(PATH + "/description/Australia")
                        .contentType(MediaType.APPLICATION_JSON)
                        .accept(MediaType.APPLICATION_JSON).header(headerName, authToken))
                .andExpect(status().isOk())
    ...
    void getRegionHandle_whenGetSwitzerlandByDescriptionContains_thenStatus200() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders
                        .get(PATH + "/description/Aust")
                        .contentType(MediaType.APPLICATION_JSON)
                        .accept(MediaType.APPLICATION_JSON).header(headerName, authToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].region", equalTo("4")))
                .andExpect(jsonPath("$[0].description", equalTo("Australia")));
    }
    ...
    void getRegionHandle_whenGetAllSwitzerlandRegions_thenStatus200() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders
                ...
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[*]").isNotEmpty())
                .andExpect(jsonPath("$[*]", hasSize(178)));
    }
    ...
    @Test
    void getRegionHandle_whenGetSwitzerlandWithoutApiKey_thenStatus200() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders
                        .get(PATH + "/region/")
                        .contentType(MediaType.APPLICATION_JSON)
