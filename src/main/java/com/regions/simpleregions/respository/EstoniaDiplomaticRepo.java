...
@Repository
public interface EstoniaDiplomaticRepo extends CrudRepository<EstoniaDiplomaticEntity, Long> {

    Optional<EstoniaDiplomaticEntity> findByRegion(@PathVariable String region);

    @Query("select estonia from EstoniaDiplomaticEntity estonia WHERE estonia.description LIKE CONCAT('%',:description,'%')")
