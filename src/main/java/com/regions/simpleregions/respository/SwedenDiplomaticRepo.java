...
@Repository
public interface SwedenDiplomaticRepo extends CrudRepository<SwedenDiplomaticEntity, Long> {

    Optional<SwedenDiplomaticEntity> findByRegion(final @PathVariable String region);

    @Query("select sweden from SwedenDiplomaticEntity sweden WHERE sweden.description LIKE CONCAT('%',:description,'%')")
