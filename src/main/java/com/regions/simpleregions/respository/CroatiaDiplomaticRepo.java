...
@Repository
public interface CroatiaDiplomaticRepo extends CrudRepository<CroatiaDiplomaticEntity, Long> {
    Optional<CroatiaDiplomaticEntity> findByRegion(@PathVariable String region);

    @Query("select croatia from CroatiaDiplomaticEntity croatia WHERE croatia.description LIKE CONCAT('%',:description,'%')")
