...
@Repository
public interface SwitzerlandDiplomaticRepo extends CrudRepository<SwitzerlandDiplomaticEntity, Long> {
    Optional<SwitzerlandDiplomaticEntity> findByRegion(final @PathVariable String region);

    @Query("select switzerland from SwitzerlandDiplomaticEntity switzerland WHERE switzerland.description LIKE CONCAT('%',:description,'%')")
